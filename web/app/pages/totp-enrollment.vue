<template>
  <div class="w-full max-w-xl bg-white shadow-lg rounded-2xl overflow-hidden">
      ...
      <div id="enrollmentFlow">
        ...
        <div class="step-indicator">
          <div class="step-item step-active">
            ...
            <span class="step-label">Setup App</span>
          </div>
          ...
          <div class="mb-8">
            ...
            <div class="mb-6">
              ...
              <div class="flex items-center justify-center mb-4">
                ...
                <button id="copySecretBtn" class="copy-btn text-gray-500 hover:text-gray-700 p-2 rounded-lg">
            ...
            <form id="verifyForm" method="POST" class="space-y-4">
              ...
              <div>
                <label for="verificationCode"
                  class="block text-sm font-medium text-gray-700 mb-2 text-center">Verification Code</label>
                <div class="flex justify-center">
                  <input type="text" id="verificationCode" name="verificationCode" maxlength="6" required
                    class="verification-code-input form-input w-48 text-center [text-indent:0.4em] py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition duration-200"
                    placeholder="000000" autocomplete="one-time-code">
                </div>
                <p id="codeError" :class="['mt-2 text-sm text-red-600 text-center', { hidden: !codeError }]">{{
                  codeError }}</p>
              </div>
...
const secretKey = useServerVar<string>('secretKey', '')
const enrollmentURL = useServerVar<string>('enrollmentURL', '')
const codeError = useServerVar<string>('codeError', '')
...
const showStep = (stepNumber: number) => {
  ...
  document.querySelectorAll('.step-item').forEach((item, index) => {
    ...
    item.classList.remove('step-active', 'step-completed', 'step-inactive')
    if (stepIndex === stepNumber) item.classList.add('step-active')
    else if (stepIndex < stepNumber) item.classList.add('step-completed')
    else item.classList.add('step-inactive')
  })
...
onMounted(() => {
  ...
  verifyForm?.addEventListener('submit', (e) => {
    ...
    if (code.length !== 6) {
      if (codeErrorEl) {
        codeErrorEl.textContent = 'Please enter a 6-digit verification code.'
    ...
    if (!/^\d+$/.test(code)) {
      if (codeErrorEl) {
        codeErrorEl.textContent = 'Verification code must contain only numbers.'
        codeErrorEl.classList.remove('hidden')
      }
      e.preventDefault()
    }
  ...
  if (hash) {
    ...
    if (stepMatch?.[1]) {
      ...
      if (stepNumber >= 1 && stepNumber <= 3) initialStep = stepNumber
    }
  }
  ...
  if (QRCode?.toCanvas && canvas && enrollmentURL.value) {
    QRCode.toCanvas(canvas, enrollmentURL.value, { width: 250 })
  }
})
</script>
