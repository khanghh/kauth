...
func setupWebRoutes(router fiber.Router, deps *webDependencies) {
	...
	router.Get("/two-factor", accountHandler.GetTwoFactorSettings)
	router.Get("/two-factor/totp/enroll", accountHandler.GetTwoFactorTOTPEnroll)
	...
	router.Get("/auth/2fa/challenge", twofactorHandler.GetChallenge)
	router.Post("/auth/2fa/challenge", twofactorHandler.PostChallenge)
	router.Get("/auth/2fa/otp/verify", twofactorHandler.GetVerifyOTP)
	router.Post("/auth/2fa/otp/verify", twofactorHandler.PostVerifyOTP)
	router.Get("/auth/2fa/totp/verify", twofactorHandler.GetVerifyTOTP)
	router.Post("/auth/2fa/totp/verify", twofactorHandler.PostVerifyTOTP)
